...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
    ...
    if(g_hWnd==NULL)
    {
        acl_error("something wrong in initWindow\n");
    }
    /* */
...
void putImageTransparent(ACL_Image *pImage,int x,int y,int width,int height, ACL_Color bkColor)
{
	...
	if(height == -1)height = pImage->height;
	//SetStretchBltMode(g_hmemdc,COLORONCOLOR);
	/*TransparentBlt is changed by me*/
...
/*this function is coded by  */
int getWndXpos()/*get window left position*/
{
    RECT inf;
    BOOL jdg;
    jdg=GetWindowRect(g_hWnd,&inf);
    if(jdg==FALSE)
        acl_error("Wrong in getWndXpos!\n");
    return inf.left;
}
int getWndYpos()/*get window top position*/
{
    RECT inf;
    BOOL jdg;
    jdg=GetWindowRect(g_hWnd,&inf);
    if(jdg==FALSE)
        acl_error("Wrong in getWndYpos!\n");
    return inf.top;
}

void drawPicture(ACL_Image *pic,int x,int y)/*draw picture in (x,y)*/
{
    beginPaint();
    putImage(pic,x,y);
    endPaint();
}
void drawPictureScale(ACL_Image *pic,int x,int y,int width,int height)
{
    beginPaint();
    putImageScale(pic,x,y,width,height);
    endPaint();
}
